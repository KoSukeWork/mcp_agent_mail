...
- 永久删除不用的 Agent：在“永久删除智能体”表中核对项目和名称，点击“永久删除”，输入 `DELETE Agent名称`（例如 `DELETE SageBay`）和当前管理员密码后确认。未绑定可信客户端的旧 Agent 也会列出。

永久删除不可通过网页撤销：删除该 Agent、它发出的邮件（含其他收件箱中的记录）、其收件记录、文件预留、联系人关系、会话绑定与身份申请。他人发送的邮件会保留，指向已删邮件的回复关联会解除；其他 Agent、客户端、审计和旧归档文件不删除。这不是擦除所有历史副本；需要保留在线数据时请先备份数据库。升级镜像本身不会删除任何 Agent。
