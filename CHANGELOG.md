...
- **npm-distributable Codex adapter** -- standalone Node.js STDIO-to-HTTP bridge, TOML profiles, read-only `--check`, native OS credential persistence and per-endpoint first-start locking. No Python/uv runtime dependency. Reuses Windows Python adapter credentials through exact read-only lookup; preserves trusted metadata across tools/resources, JSON/SSE responses and process restarts. Includes npm packaging and an AI installation prompt; no public registry publication is implied.
