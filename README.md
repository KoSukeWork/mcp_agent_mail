...
Think of it as asynchronous email + directory + change-intent signaling for your agents, with SQLite as the authoritative mailbox store. Messaging, activity, deletion, lifecycle management, and retention reporting no longer write or require a Git archive. Git remains a source-control and source-repository Guard concern; retained mailbox archives are read-only migration inputs. The former storage-layer Git repository cache, commit queue, archive browser/readers, lock repair, and bundle backup/restore primitives have been removed rather than left as unreachable compatibility code.
...
- Browser identity confirmation requires HTTPS for non-loopback origins by default. Trusted-LAN deployments may explicitly set `IDENTITY_CONFIRMATION_ALLOW_INSECURE_HTTP=true` with an `http://` base URL; startup emits a security warning because an on-path peer can intercept or alter confirmation challenges. This opt-in does not weaken URL credential, query, fragment, TTL, Origin, generation, or single-use checks.
- The logged-in web administrator is the service-level authority. Open **Identity administration** at `/mail/admin/identity` to review recovery/transfer requests, revoke or restore clients, revoke bindings, inspect the audit log, and change the administrator account. Identity actions require the current administrator password and CSRF-protected login. Restoring a client does not reactivate revoked bindings.
- Any authenticated MCP client may submit `recover_agent_identity`; only the logged-in web administrator can approve recovery. No CLI grant or MCP admin scope is needed. Approval preserves the Agent ID and messages, revokes the old binding and service credential, and increments the binding generation. Neither a challenge link nor native MCP confirmation can approve recovery on its own.
- Initial web credentials come from `MAIL_UI_USERNAME` / `MAIL_UI_PASSWORD`. After changing the account in the web UI, its salted password hash is stored in the database and takes precedence over these bootstrap settings. All old browser sessions are revoked immediately. Keep `MAIL_UI_SESSION_SECRET` configured. For a forgotten password only, use `mcp-agent-mail identity reset-web-admin` on the server; it prompts without exposing the password in command arguments and takes effect without a restart.
